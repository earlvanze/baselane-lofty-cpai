...
DEST_RULES = [
    ...
    ('88 MADISON AVE', 'NY/88 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
    ('88 MADISON OPERATIONS', 'NY/88 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
    ('88 MADISON RESERVES', 'NY/88 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
    ('86 MADISON AVE', 'NY/86 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
    ('84 MADISON AVE', 'NY/84 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
    ('90 MADISON AVE', 'NY/90 Madison Ave Public/07 - P&L & Owner Statements/Bank Statements/{year}'),
...
def remove_empty_misplaced_statement_parents(start: Path, stop: Path) -> list[Path]:
    removed = []
    cur = start
    stop = stop.resolve()
    allowed_names = {"Public", "07 - P&L & Owner Statements", "Bank Statements"}
    while True:
        try:
            resolved = cur.resolve()
            resolved.relative_to(stop)
...
def misplaced_property_statement_files(
    re_base: Path,
    month_tok: str | None = None,
    year: int | None = None,
    files: list[Path] | None = None,
    *,
    ...
    if files is not None:
        ...
        candidates = list(iter_statement_files(re_base))
    elif month_tok is not None and year is not None:
        candidates = list(iter_matching_statement_files(re_base, month_tok, year))
    else:
        candidates = []
    misplaced = []
    for source in candidates:
        if "Captured" in source.parts:
            continue
        try:
            rel_parts = source.relative_to(re_base).parts
        except ValueError:
            continue
        if not rel_parts or rel_parts[0].upper() != "NY":
            continue
        if has_legacy_lfty_segment(source.parent, re_base):
            continue
        if "Financials" in source.parts and "Bank Statements" in source.parts:
            continue
        dest_dir = resolve_dest(re_base, source.name, statement_target_year(source, year or dt.date.today().year))
        if not dest_dir:
            continue
        try:
            if source.parent.resolve() == dest_dir.resolve():
...
def move_misplaced_property_statements(
    re_base: Path,
    month_tok: str,
    year: int,
    *,
    ...
    removed_dirs = []
    skipped = []
    for source in misplaced_property_statement_files(
        re_base,
        month_tok,
        year,
        all_statement_months=all_statement_months,
        ...
        dest_dir = resolve_dest(re_base, source.name, statement_target_year(source, year))
        if not dest_dir:
            skipped.append({"source": str(source), "reason": "target-unresolved"})
            continue
        ...
        target, action = move_statement_file(
            source,
            target,
            "delete-duplicate-misplaced-property-statement-source",
            "move-misplaced-property-statement",
            "move-misplaced-property-statement-with-timestamp-suffix",
        )
        removed = remove_empty_misplaced_statement_parents(source_parent, re_base)
        removed_dirs.extend(removed)
        moved.append(
            {
                "source": str(source),
...
def planned_misplaced_property_statement_moves(
    re_base: Path,
    month_tok: str,
    year: int,
    files: list[Path] | None = None,
    *,
    ...
    for source in misplaced_property_statement_files(
        ...
        target_exists = target.exists()
        same_size = target_exists and target.stat().st_size == source.stat().st_size
        action = "delete-duplicate-misplaced-property-statement-source" if same_size else (
            "move-misplaced-property-statement-with-timestamp-suffix" if target_exists else "move-misplaced-property-statement"
        )
        planned.append(
            {
                "source": str(source),
                ...
                "source_size": source.stat().st_size,
                "target_size": target.stat().st_size if target_exists else None,
                "bucket": "misplaced-property-statement",
            }
        )
    return planned
...
def classified_issue_records(report: dict[str, Any], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for bucket, severity in [
        ...
        ("planned_misplaced_property_statement_moves", "high"),
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    misplaced_property_statements = planned_misplaced_property_statement_moves(
        args.real_estate,
        month_tok,
        args.year,
        all_existing if cleanup_all_statement_months else existing,
    ...
    planned_move_count = (
        ...
        + len(legacy)
        + len(old_financials)
        + len(old_financials_bank_files)
        + len(misplaced_property_statements)
    )
    ...
    report: dict[str, Any] = {
        ...
        "planned_misplaced_property_statement_moves": misplaced_property_statements,
        ...
        "misplaced_property_statement_move_count": len(misplaced_property_statements),
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    ...
    misplaced_property_statements, misplaced_property_statement_removed_dirs, misplaced_property_statement_skipped = move_misplaced_property_statements(
        args.real_estate,
        month_tok,
        args.year,
        all_statement_months=cleanup_all_statement_months,
    ...
    all_removed_dirs = (
        removed_dirs
        + old_financials_removed_dirs
        + old_financials_bank_file_removed_dirs
        + misplaced_property_statement_removed_dirs
        + empty_legacy_removed_dirs
        + empty_old_financials_bank_removed_dirs
    )
    print(f'MOVED={len(moved)} LEGACY_LFTY_MOVED={len(legacy)} OLD_FINANCIALS_MOVED={len(old_financials)} OLD_FINANCIALS_BANK_FILES_MOVED={len(old_financials_bank_files)} MISPLACED_PROPERTY_STATEMENTS_MOVED={len(misplaced_property_statements)} EMPTY_LEGACY_DIRS_DELETED={len(empty_legacy_removed_dirs)} EMPTY_OLD_FINANCIALS_BANK_DIRS_DELETED={len(empty_old_financials_bank_removed_dirs)} EMPTY_DIRS_DELETED={len(all_removed_dirs)} PERSONAL={len(personal)} HOLDINGS={len(holdings)} UNMAPPED={len(unmapped)} CAPTURED_UNIQUE={len(canon)}', file=stdout)
    for p in moved:
    ...
    for item in misplaced_property_statements:
        print(f'MISPLACED_PROPERTY_STATEMENT\t{item["target"]}', file=stdout)
    ...
    for item in misplaced_property_statement_skipped:
        print(f'MISPLACED_PROPERTY_STATEMENT_SKIPPED\t{item["source"]}\t{item["reason"]}', file=stdout)
    for p in personal:
