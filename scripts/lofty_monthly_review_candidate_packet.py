...
def format_reconciled_money(v,status):
    return format_money(v) if status=='ok' and parse_money(v) is not None else 'Pending reconciliation'

@lru_cache(maxsize=1)
def mortgage_escrow_property_keys():
    payload=read_json(MORTGAGE_TERMS)
    loans=payload.get('loans') if isinstance(payload,dict) else []
    return {norm(row.get('property')) for row in loans or [] if isinstance(row,dict) and row.get('property')}

def requires_mortgage_escrow_reconciliation(property_name):
    return norm(property_name) in mortgage_escrow_property_keys()
...
def has_verified_financial_summary(s):
    ...
    Column E is an internal ledger control total.  Its presence alone is not
    authority to publish an ECO Net DAO Funds figure because it can include
...
def append_monthly_financial_summary(text,s):
    ...
    text=ECO_OPERATING_CASH_SENTENCE_RE.sub('',text)
...
def render_owner_update_key_items(s,run_month=None):
    ...
    below_line=[]
    if debt: below_line.append(f"debt service of {format_money(abs(debt))}")
    if capex: below_line.append(f"capital expenditures or private-loan payments of {format_money(abs(capex))}")
    if retained: below_line.append(f"Retained Capital of {format_money(abs(retained))}")
    if below_line:
        lines.append(f"- Below NOI, the ledger records {', '.join(below_line)}; Net Operating Cashflow after these items was {format_money(nocf)}. Lofty's live NOI may differ because its Other Expenses/Utilities presentation can include these below-the-line items.")
    else:
...
def ledger_cash_flow_snapshot(s,run_month):
    ...
    b={}; revenue_row_count=0; has_short_term_rent_revenue=False
    with path.open(newline='',encoding='utf-8-sig',errors='replace') as h:
    ...
    if not has_short_term_rent_revenue and scheduled_rent is not None and scheduled_rent > 0 and rev > round(scheduled_rent*1.25,2):
...
def financials_md_summary_sections(financial_text, run_month=None):
    ...
    return sections
def owner_financials_summary_sections(financial_text,run_month=None):
    sections=financials_md_summary_sections(financial_text,run_month)
    if not sections: return []
    lines=[]
    found_net_operating_cashflow=False
    for raw in sections[0].splitlines():
        lines.append(raw)
        if re.match(r"^\|\s*Net Operating Cashflow\s*\|",raw.strip(),re.I):
            found_net_operating_cashflow=True
            break
    if not found_net_operating_cashflow:
        raise ValueError('Cash Flow Snapshot missing Net Operating Cashflow')
    owner_sections=["\n".join(lines).strip()]
    # The cash-position section is the investor-facing custody reconciliation.
    # Include it in UPDATES.md/listing summaries, but continue to omit Source
    # Evidence and the internal Column E control total.  Without this section an
    # owner sees operating results but cannot tell whether ECO actually holds
    # spendable DAO cash or has advanced cash on the DAO's behalf.
    if len(sections) >= 2 and re.match(r"(?i)^##\s+Monthly Cash Position\b", sections[1].strip()):
        owner_sections.append(sections[1].strip())
...
def live_publish_exclusion(property_path,manual_excluded_properties=None):
    manual_names=[*DEFAULT_MANUAL_EXCLUDED_PROPERTIES,*(manual_excluded_properties or [])]
    guards,_,_=monthly_exclusion_guards(None,manual_names)
...
def prune_unlisted_candidates(output_dir:Path,run_month:str,records):
    """Remove only regenerated candidate files omitted by the current manifest.

    A month directory is reused across runs. Without this bounded cleanup,
    properties removed from the current manifest leave plausible-looking but
    stale financial summaries beside the authoritative candidates.
    """
    if not records or not re.fullmatch(r'\d{4}-\d{2}',str(run_month or '')):
        return []
    allowed={str(Path(str(r.get(k))).resolve()) for r in records for k in ('update_candidate','financial_candidate') if str(r.get(k) or '').strip()}
    removed=[]
    for candidate in output_dir.glob(f'*/{run_month}-*-review-candidate.md'):
        if str(candidate.resolve()) in allowed:
            continue
        try:
            candidate.unlink(); removed.append(str(candidate))
            try: candidate.parent.rmdir()
            except OSError: pass
        except OSError:
            continue
...
def build_packet(manifest,output_dir:Path,entry_date:date,lofty_profile_json=None,source_ledger=None,lofty_properties_dir=None,lofty_all_properties_json=None,dao_cash_report=None,reporting_cutoff_date=None,manual_excluded_properties=None):
    ...
    for r in manifest_records:
        ...
        if summary.get('eco_gl_column_e_source_mode')=='source_ledger_zero_rows' and summary.get('eco_gl_column_e_row_count')==0:
            ...
            rec['zero_row_source_ledger_reason']='active monthly manifest record has no property-split GL and no matching source-ledger rows; do not infer ECO Net DAO Funds from the empty ledger'
...
def render_markdown(report):
    ...
    return '\n'.join(lines)
def main(argv=None):
    ap=argparse.ArgumentParser(description='Generate clean review candidate files from monthly Lofty drafts without marking them approved.'); ap.add_argument('--manifest',required=True,type=Path); ap.add_argument('--output-dir',required=True,type=Path); ap.add_argument('--entry-date',required=True,type=parse_iso_date); ap.add_argument('--reporting-cutoff-date',type=parse_iso_date,default=None); ap.add_argument('--lofty-profile-json',type=Path,default=None,help='Optional PM/SDK-derived profile snapshot; do not pass LoftyAssist snapshots.'); ap.add_argument('--lofty-properties-dir',type=Path,default=None,help='Optional PM/SDK-derived property snapshots; do not pass LoftyAssist snapshots.'); ap.add_argument('--lofty-all-properties-json',type=Path,default=None,help='Optional PM/SDK-derived all-properties snapshot; do not pass LoftyAssist snapshots.'); ap.add_argument('--source-ledger',type=Path,default=default_source_ledger()); ap.add_argument('--dao-cash-report',type=Path,default=DEFAULT_DAO_CASH_REPORT); ap.add_argument('--manual-excluded-property',action='append',default=[]); ap.add_argument('--report',required=True,type=Path); ap.add_argument('--markdown',required=True,type=Path); a=ap.parse_args(argv)
    ...
    report=build_packet(json.loads(a.manifest.read_text(encoding='utf-8')),a.output_dir,a.entry_date,profile,a.source_ledger if a.source_ledger.exists() else None,properties_dir,all_properties,a.dao_cash_report,a.reporting_cutoff_date,a.manual_excluded_property)
